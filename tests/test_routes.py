...
class TestAccountService(TestCase):
    ...
    def setUpClass(cls):
        ...
        # Disable force_https to allow testing with HTTP
        talisman.force_https = False

    @classmethod
    def tearDownClass(cls):
        """Runs once after all tests"""
        pass  # Add any teardown logic here if needed
    ...
    def _create_accounts(self, count):
        ...
        for _ in range(count):
            ...
            response = self.client.post(
                "/accounts", json=account.serialize()
            )
            self.assertEqual(
    ...
    def test_bad_request(self):
        ...
        response = self.client.post("/accounts", json={"name": "not enough data"})
        # Expecting a 400 status, check for 400
